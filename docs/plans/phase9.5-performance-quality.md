...
## §9. ADR起票方針（ADR-0064、正式起票済み）

**正式起票済み（2026-08-12、DECISIONS.md）**: 起票直前に`grep -n "^### ADR-" DECISIONS.md | tail -3`を再実行し、最新確定ADRが**ADR-0063**（Phase 9）であることを確認したうえで、本計画の決定群を**ADR-0064**として正式起票した（`DECISIONS.md` L1666〜）。当初は「M/F-1/F-2/F-3実装完了時点で1本、PR-1/PR-2の採否確定時点で追加要否を判断」という2段階起票を想定していたが、実際にはPR-1（GPU=NO_GO確定）・PR-2（縮退確定）を含む全マイルストーン完了を待って**1本にまとめて起票**した（決定内容が相互に参照し合う一つの計測駆動フェーズの成果であるため）。記録した決定（確定、実装・実機検証反映後）:
1. F-4（Recovery pairing検証の交差一致緩和）・F-5／F-5b（Planウォームゲート自爆の二層修正）
2. F-1／F-1b（few-shotイベントカテゴリ条件選択）は実機A/B実測2連続陰性により不採用・ロールバック確定（F-1c）。カテゴリ推定を`ai/prompt/`配下の状態レス純関数（`EventCategoryClassifier`、ja/en別辞書、PII非送信）として新設したこと自体はドーマント基盤として残置
3. Engineウォームアップの入口は`LocalAiGateway.warmUp()`とし、既存の事前ガード列（aiEnabled/Tier/ABI/モデル解決/availMem）をすべて経由してからadapterのEngine準備を呼ぶ（features層はGatewayのみ参照する既存層規律を維持、敵対的レビューA-5）。ガードのavailMem閾値は当初、通常推論（+512MB）より厳しい`WARM_UP_EXTRA_HEADROOM_BYTES`＝+1GiB（敵対的レビューA-6）としたが、実機検証で6GB端末の常閉の門と判明しF-2bで**+512MB（通常ガードと同値）へ再調整**した。トリガーはトップ画面（`EventSelection`）入場時のみ・都度・in-flight時スキップに確定する（敵対的レビューA-7）
4. F-3（Recovery用maxNumTokensプロファイル縮小）は構造分析のみで本番配線不成立と裁定（縮退）。見積もり関数自体はドーマント設計文書として残置
5. `AiMetrics`へのPSSフィールドは追加しない（§12確認事項4で確定。probe専用`PssPeakSampler`で計測を完結させ、Phase 12のAnalytics基盤設計時に本番計測の要否を再検討する）
6. PR-1（GPUバックエンド）は実機検証によりNO_GO確定（A54はOpenCLライブラリをアプリへ非公開）。本採用は行わない
7. PR-2（マスク模範例・カテゴリ条件付きecho検出）はプローブ未実施のまま縮退確定。Phase 12での再評価へ申し送る

**再検討トリガー**: (a) F-2実装後のA/B実測でウォームアップが正味の遅延・電池消費の悪化になっていることが判明した場合、トリガー画面の見直しまたはF-2自体の撤回を検討する。(b) `onTrimMemory`連動でのウォーム済みEngineの能動的アンロード（本フェーズでは非実装、§3.3敵対的レビューA-6）は、F-2実機運用でメモリ圧迫の実害が観測された場合に導入を検討する。ADR-0064はこれに加え、KVキャッシュAPI追加時のF-3再評価・`onTrimMemory`実装時のF-2bヘッドルーム再評価・GPU delegate経路追加時のPR-1再実行・Phase 12品質定量化環境整備時のPR-2/F-1再評価の4件を再検討トリガーとして記録している（`DECISIONS.md` ADR-0064「再検討トリガー」参照）。
