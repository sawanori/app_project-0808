...
| **P6-C7** 薄いG4-E | Simulate delay → Recovery到達・候補ETA表示を ja/en スクリーンショット取得（T-P6E2E-1）、Use this plan復帰の非ループ確認（T-P6E2E-2） | quality-runner | **G4-E** |**完了（実測2026-08-10、quality-runner。詳細な実行環境・全ケース一覧・重大インシデント記録は`docs/plans/phase5-notification-execution.md`§10.11を参照——本G4-Eサイクルは同一emulatorセッションでPhase 5（T-P5E2E-1〜4）と合同実行した）**: `RecoveryBasicE2ETest`の`tP6E2e1_simulateDelay_reachesRecoveryWithAtLeastOneOptionShowingEta`・`tP6E2e2_useThisPlan_returnsToExecutionWithoutLoopingBackToRecovery`とも**PASS**（`tests="2" failures="0"`実測、`build/agent-logs/g4e-A5-RecoveryBasicE2ETest-result.xml`）。「次の予定」共有seed（Phase 2 §12.1手順・account`g4e@local`）＋READ_CALENDAR/POST_NOTIFICATIONS/ACCESS_FINE_LOCATION`granted=true`実測確認済みの状態で実行。T-P6E2E-1はSimulate delay→Recovery到達後、`RecoveryViewModel.init`の非同期`createRecoveryPlan`ロード完了（`waitUntil`15秒ポーリング、実機では約1〜7秒で観測）を経て候補1件以上・ETA表示を確認。T-P6E2E-2は候補選択→「Use this plan」→Execution復帰・Recoveryへの再ループなしを確認。<br><br>**ja/enスクリーンショット取得**（T-P6E2E-1）: 自動テストとは別に、`am start`＋`uiautomator dump`のtext一致で座標特定した`input tap`による手動UI操作でPrepare→Start→「Simulate delay (debug)」の順に遷移し、`adb exec-out screencap -p`で取得。`docs/evidence/screenshots/phase6/en/01-recovery.png`（"Continue as planned"候補＋"ETA 12:52 AM"、1080×2400）・`phase6/ja/01-recovery.png`（"予定どおり進める"＋"到着予定 0:54"、1080×2400）。両ロケールとも「候補ロード直後は"No alternatives available"の初期空状態が数秒表示され、非同期ロード完了後に候補が反映される」という実機特有の過渡状態を実測確認した（`RecoveryViewModel.init`のKDoc記載どおり、フレーク回避のため待機を要する設計）。ロケール切替は`cmd locale set-app-locales com.actionstarter --locales ja`→取得後`--locales en`で復帰、`get-app-locales`で最終`[en]`を実測確認。<br><br>**cleanup**: seedカレンダー（`g4e@local`）を削除し`calendars`/`events`/`instances`3テーブルとも空を実測確認、権限を標準granted状態へ復帰（詳細は phase5 §10.11参照）。エミュレータ・adb使用、git commitなし、APIキー値の出力なし。 |
