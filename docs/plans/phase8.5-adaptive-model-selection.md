...
> 承認状態: **完了（2026-08-11）。F-A Green・コミット済み（9ec7218）。F-B Green・コミット済み（2fcb8ae、ADR-0062起票、tests=681/skipped=1/failures=0、lint error=0）。§9実機受け入れは§12に結果記録済み（自動選択の配線・AI文言描画は合格。Qwen0.6Bのfew-shotエコー品質欠陥をPhase 9/12へ申し送り）**
...
## §12. A54実機受け入れ結果（2026-08-11実施）

**実施環境**: 前回§9受け入れ（`phase8-a54-ram-tier-fix.md`§10）と同一Galaxy A54 5G（SCG21）・日常使用状態。ワイヤレスデバッグ接続維持。アプリはコミット2fcb8ae（F-B、本計画書コミット2）ビルドを`installDebug`（`lastUpdateTime`実測17:27）。証拠スクリーンショットは`docs/evidence/screenshots/phase8/a54-p85-*.png`に格納。

### 12.1 Settings新UI実機描画（§9手順2対応）

- 「自動（推奨）」行＋説明文「端末の空きメモリに応じて、Gemma 4またはQwen 3のいずれかを自動的に選びます。」（§11確認事項2の実機表示確認）。
- Gemma4行・Qwen行それぞれ個別の状態表示（両方ダウンロード済み・検証済み）。
- **Gemma4行にメモリ不足注記（赤字）が表示**（§11確認事項1の実機動作確認）。
- **Qwen行に「この端末におすすめ」推奨バッジ**（TIER_1推奨、T-P85-18の実機動作確認）。
- 既存の明示選択（qwen）のラジオ選択状態が保持されている（§11確認事項3の実機動作確認）。

スクショ`a54-p85-settings.png`。

### 12.2 メモリ不足注記の境界動作（重要な観察）

Settings画面を開いた時点で、Gemma4行だけでなくQwen行にも注記が表示された。直後の実測availMemは1,909,112kB≈1.82GiBで、Qwenの要求量1.75GiB（`defaultProfilePeakRamBytes`1.25GiB＋`MEMORY_SAFETY_MARGIN_BYTES`512MB）との差はわずか72MB。コード自体は`refresh()`が設計どおり`defaultProfilePeakRamBytes + 512MB`で判定しており実装バグではないが、開画面時点ではこの僅差の範囲でしきい値を割っていたとみられる「境界揺らぎ」である。日常使用状態のA54はavailMemが1.8〜2.4GiB程度で常時変動しており、Qwen0.6Bの要求量（相対的に軽量なモデル）ですら境界に触れる時間帯が実際に存在することを記録する。

### 12.3 UIから「自動」選択（§9手順4対応）

Settings画面で「自動」行をタップし、`run-as`経由で`ai_preferences.xml`を確認したところ`selected_model_id`が`auto`へ書き込まれていることを確認した。P7-C6申し送り事項（旧`SettingsViewModel.kt`L56のGemma4固定によりモデル選択UIが存在せず、前回§10.4の検証ではテスト目的の`run-as`直接編集が必要だった制約）がF-Bの実装により解消されたことの実機確認である。

スクショ`a54-p85-auto-selected.png`。

### 12.4 自動選択→AI文言描画（§9主目的・合格）

`am kill-all`でavailMemを2,297,588kB≈2.19GiBまで回復させたうえで、プラン画面へ入場した。対象は実予定「映画館18:00」（テスト予定「歯科検診」は20:00へ移動済みのため、「次の予定」単線フローにより主予定=映画館のプランが開くことを確認済み）。

**手動介入（`run-as`等）ゼロ**でCPU使用率337%の推論が走り、17:35ステップの文言がBasic「出かける準備をする」からAI生成文言へ差し替わった。availMem 2.19GiBはGemma4の要求量2.5GiBを満たさずQwen0.6Bの要求量1.75GiBのみ満たす水準であり、`ModelSelector`の候補決定表からQwen0.6Bが自動選択されたことが確定する。

スクショ`a54-p85-plan-t5.png`（Basic段階）→`a54-p85-plan-t40.png`（AI差替後）。

### 12.5 【重要発見・品質欠陥の実例】Qwen 0.6Bのfew-shotエコー

映画館プランの17:35ステップに描画されたAI文言は「**歯科検診**」（他の予定の裸の名称）だった。直前に歯科検診プラン自身へ生成された文言（「歯科検診を受ける場所を確認する」）とは異なるため、ステイルな書き込みの使い回しではなくモデル出力そのものである。few-shot例文（歯科検診系の例文）をそのまま鸚鵡返しした出力と判定する。

**再現性**: 再入場後の2回目生成でも同一出力（「歯科検診」）が再現された。本アプリのサンプリング設定（topK=1・temperature=0の決定的サンプリング）を踏まえると、これは偶発的な失敗ではなく系統的な品質問題であることの証拠である。

時刻・構造・順序（§13不変条件）は無傷のまま成立している——AI文言の**内容**のみが破綻しており、アプリの構造・スケジューリング自体には影響していない。

スクショ`a54-p85-dental-plan.png`（再入場直後・Basic段階）→`a54-p85-plan-2nd.png`（2回目生成・再現）。

### 12.6 受け入れ判定

**§9の主目的（自動選択の配線がユーザー操作の範囲内〔手動介入なし〕で機能し、AI文言が実機で描画される）は合格**。F-Aで発見・是正したモデルパス配線の構造的バグ（ADR-0062決定5）とF-BのSettings多モデルUIが、実機で意図どおりに連動して動作することを確認した。

ただし、6GB日常使用端末で自動選択が実際に選ぶQwen 0.6Bの文脈化品質に、few-shot例文の鸚鵡返しという系統的な欠陥が実例として観測された。これは配線・構造の合否とは別軸の「モデル自体の品質」問題であり、§9の主目的の合否判定には影響しないが、実用上の重要な既知課題として記録する。

### 12.7 Phase 9／12への申し送り

1. `ContentSanityChecker`へ「few-shot例文タイトルの鸚鵡返し検出」ルールの追加を検討する（例文集自体はアプリ側の既知データのため、実装は可能と見込まれる）。
2. 表示品質の実利用最低ラインを、Phase 12のBasic/AI比較検証で定量化する。
3. 品質天井を確保したい場合の代替案として、Gemma4が動作する環境（8GB機、または空きRAMに余裕がある状況）でのみAIを推奨する段階的な既定方針も選択肢として残す。

### 12.8 端末に残した状態（次回操作者向け申し送り）

`selected_model_id=auto`のまま／AI有効化ON／Gemma4・Qwen0.6Bの両モデルファイルが端末内に残存／テスト予定「歯科検診」20:00がユーザーの実カレンダーに残存（削除待ち）。実予定「映画館」に対しては開始操作を行っていない（プラン閲覧のみ、通知は未スケジュール）。
