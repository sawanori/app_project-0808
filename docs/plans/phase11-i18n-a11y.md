...
| **P11-C4** 統合／実機 | NavHost統合ウィンドウ（直列）。§8.7のゲート検証手順（実機TalkBack操作確認・実機fontScale=1.5目視確認・ja/enスクリーンショット取得） | domain-implementer（integration owner）→ quality-runner | **本タスク（ui-implementer/test-writer、P11-C1/C2/C3/C5）の範囲外。最終デバイスラウンドへ委譲**（Fable 5指示によりC4はエミュレータ・adb・実機操作を伴うため本タスクから除外）。NavHost統合自体はC3で完了済み（NavHostは共有ファイルとして本タスクが直接編集した）。§8.7の3手順（TalkBack操作確認・実機fontScale=1.5目視確認・ja/enスクリーンショット取得）が未実施のまま残っている。申し送り詳細は完了報告参照。 | **G4-E**（未達のまま） |
