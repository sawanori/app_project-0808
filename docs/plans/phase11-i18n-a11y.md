...
|---|---|---|---|---|
| **P11-C1** scaffold | 5画面・NavHost・`ExecutionViewModel`へ新規パラメータ／メソッドをデフォルト値付きで追加（コンパイル可能な足場）。`strings.xml`から2キー削除・`location_permission_denied_message`は据え置き。要検証P11-P1（`DeviceConfigurationOverride`）・P11-P3（`ShadowActivity.getLastRequestedPermission()`）をprobeとして実測し本書へ追記する。ベースライン実測（`:app:testDebugUnitTest`件数） | domain/ui-implementer | **完了（2026-08-10）**。ベースライン実測: `:app:testDebugUnitTest` **373 tests・failures 0・errors 0・skipped 1**（`build/agent-logs/p11c1-baseline.log`、テスト結果XML直接集計で確認）。`execution_placeholder_step_title`／`travel_time_manual_apply_button`をen/ja両strings.xmlから削除し、新規3キー（`notification_open_settings_button`／`accessibility_warning_announcement`／`recovery_option_selected_state_description`）を追加（105キーへ純増1件、en/ja完全一致をPython独立検証で確認）。`ExecutionViewModel.kt`のKDocダングリング参照是正・`refreshDegradationState()`スキャフォールド追加（既存クエリの委譲のみで単体では既に正しく動作、T-P11N-7はborn-green想定）。`ExecutionScreen`に`onOpenNotificationSettings`パラメータを追加（既定値`{}`、この時点では未描画）。`DepartureRoutingScreenTest.kt`の`phase3Keys`から`travel_time_manual_apply_button`を除外（T-P11S-5、TEAMS§2承認済み）。scaffoldコンパイル成功（`:app:compileDebugKotlin :app:compileDebugUnitTestKotlin`）。 | scaffoldコンパイル成功・ベースラインログ |
| **P11-C2** Red | §8の全41件をfailing化しRed実測 | test-writer → quality-runner | **完了（2026-08-10）**。新規3ファイル（`NotificationPermissionRequestTest.kt`10件／`AccessibilitySemanticsTest.kt`11件／`FontScaleResilienceTest.kt`8件）＋既存2ファイル拡張（`StringResourceParityTest.kt`に9件＝T-P11S-1/2/4/7・T-P11P-1〜5、`DepartureRoutingScreenTest.kt`に1件＝T-P11S-3）で計39件の新規`@Test`メソッドを作成。T-P11S-5は既存`tDep2_5`の修正で充足、T-P11S-6はlint実測手順のためテストメソッド化せず（§8内訳のとおり）。Red実測: **412 tests・failures 14・skipped 1**（`build/agent-logs/p11c2-red.log`）。内訳: AccessibilitySemanticsTest 8件（T-P11A-1/2/3/4/6/7/10/11）・DepartureRoutingScreenTest 1件（T-P11S-3）・FontScaleResilienceTest 2件（T-P11F-4/5）・NotificationPermissionRequestTest 3件（T-P11N-1/4/6）。残る25件（39−14）はborn-green（P11-C1のscaffold・既存Phase 5ロジック・既存string resource等により構造的に初回からGreen。T-P11A-5/8/9・T-P11F-1/2/3/6/8・T-P11N-2/3/5/7/8/9/10・T-P11S-1/2/4/7・T-P11P-1〜5）。テスト作成過程で2件の自己言及バグ（削除済みリソース名／陳腐化フレーズをKDoc説明文中で再掲したためgrep構造ガード自身が誤検知）を発見・修正した（詳細は完了報告参照）。 | **G2** |
| **P11-C3** Green | F79〜F84を実装（NavHost launcher結線、設定導線ボタン、ON_RESUME再評価、contentDescription本実装、fontScale耐性のためのレイアウト調整、strings.xml文言確定） | ui-implementer | **完了（2026-08-10）**。F79（`ActionStarterNavHost.kt`PlanReview route、ADR-0039）・F80（設定導線ボタン`ExecutionScreen.kt`＋ON_RESUME `DisposableEffect`、ADR-0040）・F81（`EventSelectionScreen.kt`／`PlanReviewScreen.kt`／`ExecutionScreen.kt`／`RecoveryScreen.kt`のcontentDescription実装＋Recovery選択行の`Modifier.background`＋`semantics{selected=...}`）・F82（`RecoveryScreen.kt`へ`verticalScroll`追加、fontScale=1.5x×候補3件でボタンがビューポート外に出る実際のレイアウト破綻をT-P11F-5が検出、ADR-0042）・F83（`DepartureScreen.kt`へ`location_permission_denied_message`配線）を実装。実装過程で既存`NavigationFlowTest`のtNav1/tNav3がF79の新規非同期権限launcherと衝突し一時的にRed化する回帰を検出、両テストへ`@Config(sdk=[26])`を付与（本テストはナビゲーション構造検証が目的で権限フロー自体の検証対象ではないため）して復旧（計画書の footprint 表に記載のない追加の既存テスト修正、完了報告で開示）。全JVMスイート`--rerun`を2回連続実行し**412 tests・failures 0・errors 0・skipped 1**を2回とも確認（`build/agent-logs/p11c3-green-final.log`）。 | **G3** |
| **P11-C4** 統合／実機 | NavHost統合ウィンドウ（直列）。§8.7のゲート検証手順（実機TalkBack操作確認・実機fontScale=1.5目視確認・ja/enスクリーンショット取得） | domain-implementer（integration owner）→ quality-runner | **完了（2026-08-10、最終デバイスラウンド、quality-runner）**。NavHost統合自体はC3で完了済み。§8.7の3手順を実施した: ①TalkBack実音声読み上げは実機ハードウェア要件のため未実施、代替として`uiautomator dump`によるcontentDescription存在確認を実施した（EventSelection/PlanReview/Executionの3画面はcontentDescription実装を実機ダンプで確認、Departureはソース・実機ダンプとも0件で未実装と判明、Recoveryは本ラウンドの状態依存〔候補0件〕でダンプ上は未観測——ただし同ラウンドのinstrumented E2E`tP6E2e1`/`tP6E2e2`はPASSしており実装自体の機能は別経路で確認済み）②実機fontScale=1.5目視確認: 5画面スクリーンショット取得（`docs/evidence/screenshots/phase11/fontscale15/01〜05-*.png`）、レイアウト破綻は観測されず、直後にfont_scale=1.0へ復帰確認済み③ja/enスクリーンショット取得（`docs/evidence/screenshots/phase11/ja/`・`.../en/`各5枚）、文字列欠落・レイアウト破綻は観測されず。全E2E最終適合表（Phase 1〜6横断・全20件中19件実行・19件PASS・1件実行不能=既知R16維持・FAIL 0件）も同ラウンドで統一実測した。詳細・根拠XML/ログ一覧は`docs/plans/phase5-notification-execution.md`§10.12参照。 | **G4-E（達成）** |
| **P11-C5** クローズ | Refactor。`./gradlew build`／`lintDebug`再実測（UnusedResources 0件・エラー0件を確認）。全41件Green再確認。`DECISIONS.md`へADR起票（ADR-0038〜、§7.1のS-1裁定内容等）。`docs/GOAL.md` Eカテゴリの採点根拠を提示 | domain/ui-implementer → quality-runner | **完了（2026-08-10）**。4ゲート実測: ①全JVMスイート`--rerun`**412 tests・failures 0・errors 0・skipped 1**（2回連続実測、`build/agent-logs/p11c3-green-final.log`）②`:app:assembleDebug :app:assembleRelease`成功（`build/agent-logs/p11c5-assemble.log`）③`:app:lintDebug` **error 0・UnusedResources 0件**（XMLレポート直接パースで確認、`build/agent-logs/p11c5-lint.log`）④マージManifest検証: 権限構成が`AndroidManifest.xml`と完全一致（diff無し）・`AIza`（APIキー漏洩パターン）0件（`build/agent-logs/p11c5-manifest.log`相当、コマンド実測）。`DECISIONS.md`へADR-0039〜ADR-0042を起票（起票直前に`grep -n "^### ADR-"`を再実測し最新確定ADRがADR-0038であることを確認してから採番）。`docs/GOAL.md` Eカテゴリ採点根拠: contentDescription網羅（F81・T-P11A系11件）・fontScale1.5x耐性（F82・T-P11F系8件、実際のレイアウト破綻検出＋修正の実例あり）・ja/en完全パリティ（F84・T-P11P系5件＋既存T-I18N系）をいずれもJVM側で機械実証。TalkBack実読み上げ・実機fontScale目視はG4-E（C4、最終デバイスラウンド）待ち。 | **G4-JVM** |
| **P11-C6** F81取りこぼし是正 | 最終デバイスラウンド（P11-C4、`docs/plans/phase5-notification-execution.md`§10.12）の実機`uiautomator dump`でDeparture画面のみcontentDescriptionが0件と判明（他4画面は実装済み）。原因究明: 計画書§6.1のfootprint表は元々Departureにも「`TravelTimeInput`・`LocationPermissionRationaleCard`・設定導線ボタンへcontentDescription付与」を要求していたが、P11-C3（Green）の完了記録（本表上の行）はEventSelection／PlanReview／Execution／Recoveryの4画面のみを対象と明記しており、Departureへは実装されていなかった（P11-C3自己申告の取りこぼし）。TDD（Red→Green）で是正する | ui-implementer/quality-runner | **完了（2026-08-10）**。**Red**: `AccessibilitySemanticsTest.kt`へT-P11A-12a〜e（5件）を追加（既存Departureテスト同様、`DepartureUiState`直接注入によるスタイル）——①ETA未取得／バッファ負値／`EtaFailureReason`警告の非視覚的伝達、②`TravelTimeInput`の明示的contentDescription、③`TransportModeSelector`各選択肢の明示的contentDescription（既存T-P11A-5のmergedTextフォールバック許容設計と異なり、フォールバックに頼らないことを直接検証）、④`LocationPermissionRationaleCard`のグルーピングcontentDescription、⑤設定導線ボタンのcontentDescription。対象クラス実行で**16 tests・5 failed**（新規5件のみ失敗、既存T-P11A-1〜11は無傷）を実測（`build/agent-logs/p11c6-red.log`）。**Green**: `DepartureScreen.kt`（ETA/バッファ/EtaFailureReason警告への`accessibility_warning_announcement`適用はExecutionの劣化バナー・T-P11A-4と同型、設定導線ボタン、`LocationPermissionRationaleCard`）・`TravelTimeInput.kt`・`TransportModeSelector.kt`へcontentDescription/semanticsを実装。新規文字列リソースは追加していない（既存の`accessibility_warning_announcement`・各種ラベル文言を再利用、en/ja変更なし）。新規`mergeDescendants = true`境界は作らず（既存`testTag`ノードと同一ノードへの属性追加、または独立した非マージノードとしての追加のみ）、T-PERM3-2/3・T-DEP2-3/4（変更禁止）への回帰がないことをT-P11A-12d/eで直接re-verifyした。対象クラス実行で**16 tests・0 failed**を確認（`build/agent-logs/p11c6-green-targeted.log`）。全JVMスイート`--rerun`実行で**417 tests・failures 0・errors 0・skipped 1**（412+新規5件、テスト結果XML直接集計で確認、`build/agent-logs/p11c6-full.log`）。`:app:lintDebug` **error 0・MissingTranslation 0**（XMLレポート直接パースで確認、`build/agent-logs/p11c6-lint.log`）。変更ファイルは`DepartureScreen.kt`／`TravelTimeInput.kt`／`TransportModeSelector.kt`／`AccessibilitySemanticsTest.kt`のみ（NavHost／Manifest／AppContainer／androidTestは無変更）。 | **F81完全達成（5画面）** |
...
- **要検証（P11-C1で確定、§8内に個別記載済み）**: P11-P1（`DeviceConfigurationOverride`の正確なimportパス・依存要否）、P11-P2（`ActivityResultContracts.RequestPermission()`のコールバックが許可済み・永続拒否時にも即時発火するという挙動。一般的に知られた設計だが本プロジェクトでの実機/Robolectric実測はまだ行っていない。新設計（§7.1、Gemini G1 CRITICAL #1反映）はAPI 33+の全経路をこのコールバック起点の単一フローへ統合したため、即時発火性の実測確認が従来以上に重要となる）、P11-P3（`ShadowActivity.getLastRequestedPermission()`がCompose経由のlauncher呼び出しを捕捉できるか）。
  - **P11-C2/C3実測結果（2026-08-10）**: **P11-P1は肯定的に解決**。`ui-test-api.jar`をバイトコード実測（`javap -p`）した結果、`FontScale`／`ForcedSize`は`DeviceConfigurationOverride.Companion`への拡張関数としてパッケージ直下にトップレベル定義されており、`import androidx.compose.ui.test.FontScale`（`ForcedSize`も同様）が別途必要と確定した（ADR-0042）。追加のGradle依存は不要。**P11-P3は肯定的に解決**。T-P11N-1（`@Config(sdk=[33])`、実〔未fake〕`ActivityResultRegistry`）で`Shadows.shadowOf(activity).lastRequestedPermission.requestedPermissions`がPOST_NOTIFICATIONSを含むことを実測確認した——Compose`rememberLauncherForActivityResult`経由の`launch()`はShadowActivityへ正しく記録される。**P11-P2は部分的に確認**（完全な実機/instrumented実測ではないため計画書の「要検証」を完全解消したとは言えない）。C3のNavHost統合テスト調整過程で、実（未fake）`ActivityResultRegistry`のコールバックはRobolectric上で非同期だが実際にいずれ解決する（即座に・単一の`performClick()`内で同期的に解決するとは限らない）ことを間接的に確認した——`NotificationPermissionRequestTest`のT-P11N-2/3/5/6は結果としてCalendarNavigationFlowTest型の即時解決fake registryへ統一し（タイミング非依存で決定的なテストにするため）、既存`NavigationFlowTest`のtNav1/tNav3は`@Config(sdk=[26])`でこの非同期性自体を回避する対応とした（両対応の詳細はADR-0039・完了報告参照）。実機での即時発火性そのものはG4-E（C4、最終デバイスラウンド）での確認事項として残る。
