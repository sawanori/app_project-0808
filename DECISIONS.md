...
### ADR-0063: Recovery文言をLocal AI化する（`LocalAiRecoveryContextualizer`overlay decorator）とともに、Plan/Recovery共通の品質防御ハーネス（few-shotエコー検出R1a・最小品質ヒューリスティックR2）を導入する（Phase 9、仕様§73）

- 日付: 2026-08-11 ／ ステータス: 採用（`docs/plans/phase9-recovery-ai.md`のユーザー承認・敵対的レビュー反映後に実装着手、コミット1〜3実装完了＋A54実機受け入れ合格（同計画書§14）をもって確定） ／ 決定者: domain-implementer（Step 4実装完了・A54実機受け入れ合格をオーケストレーターが確認した時点の判断） ／ 起案agent: domain-implementer ／ 関連仕様§: §73「Phase 9・Local AI Recovery」・§32「Recovery Option」・§33「Recoveryの優先原則」・§34「ユーザー最終決定」・§15「LLMに禁止すること」・§19「AI OFF時でも動作すること」・§20「Structured Output」・§21「AI Promptの言語非依存化」・§61「MVPに入れない機能」・§88「Developer UX Principle」、ADR-0045（`generatePlan`生JSON文字列契約）、ADR-0047（`SchemaValidator`／`ContentSanityChecker`の①②責務分離）、ADR-0049・ADR-0050（retry契約・Analytics先送り）、ADR-0053（`ModelStorage`配置規約）、ADR-0057（few-shot拡張・`estimateMaxNumTokens`）、ADR-0062（`ModelSelector`自動選択・`modelPath`引き回し）

- **ADR番号の付番根拠**: 起票直前に`grep -n "^### ADR-" DECISIONS.md | tail -3`を再実行し、最新確定ADRがADR-0062（本書1601行、Phase 8.5 F-B）であることを確認した。計画書§9の起案時点の前提（「最新確定ADRはADR-0062」）と一致しており、齟齬はない。

**背景**: Phase 8.5§12.5の実機実測（A54、Qwen3-0.6B自動選択）で、無関係な「映画館」の予定に対しAIが模範例のタイトル「歯科検診」をそのままエコーする実例が観測されていた。Phase 9はこのエコー実例を出発点に、(a) Plan側で既に確立済みの overlay decorator パターン（Phase 8 B1裁定）をRecoveryへ同型展開し、(b) エコー実例が示した具体的な欠陥に対する品質防御ハーネス（L2: few-shotエコー検出R1a・最小品質ヒューリスティックR2）をPlan/Recovery両経路の共通基盤として新設する、という2本柱で構成した。計画段階でオーケストレーター＋Gemini 3.5-flashの敵対的レビューを実施し、単一のR1ルールでは「本物の歯科検診の予定に対しAIが正当に『保険証を確認する』と出力したケース」まで誤って弾いてしまうという最重要指摘（両レビュー一致）を受け、R1をR1a（模範例タイトルの裸のecho検出、Phase 9スコープ）とR1b（modelStepsのdisplay_text echo検出、カテゴリ一致判定を要するためPhase 9.5送り）へ分割した（計画書§4.2・§13）。

**決定**:
1. `RecoveryEngine`を実装する新クラスを作らず、`LocalAiPlanContextualizer`と同型の`LocalAiRecoveryContextualizer`（overlay decorator）を採用する（Phase 8 B1裁定の踏襲。`RecoveryEngine`インターフェース自体は決定的Basic計算専任のまま、AIはGateway経由の非同期後差し替えとして分離する構造を維持）。
2. `AIRecoveryOptionResponse`から`skippedStepIds`／`displayText`を削除し`semanticAction`＋`explanation`のみに縮小する。`generateRecovery`の戻り値を`AIRecoveryResponse`から生JSON文字列（`String`）へ変更する（ADR-0045同型。パース・①形式検証・②内容sanity検証はいずれも`LocalAiGateway`側の責務とする既存方針をRecoveryへも踏襲）。
3. `generateRecovery`へ`modelPath`引数を追加する（ADR-0062決定5の同型是正。`LocalAiGateway`が検証・選択したモデルと`LiteRtLmLocalLanguageModel`が実ロードするモデルを構造的に一致させる）。
4. `ContentSanityChecker`へL2 R1a（few-shotエコー検出、`checkFewShotEcho`）・R2（最小品質ヒューリスティック: 最小長／タイトルコピー禁止／動詞相当表現の存在確認）を追加し、Plan（`check`）・Recovery（新設`checkRecovery`）の両経路へ統合する。R1aは`isTitleCopy`の正規化＋完全一致／80%以上占有ロジックを再利用するが、few-shotタイトルはシステムが管理する既知の固定集合（「歯科検診」4文字・「出張」2文字を含む）であるため、`isTitleCopy`が使うtitleコピー検査の6文字未満免除閾値をそのまま流用せず、免除なし（実質的に閾値1）の別パラメータで適用する（実装時の実測発見。免除閾値をそのまま流用すると6文字未満の模範例タイトルへのエコーを一切検出できなくなることが判明したための是正）。R2(c)の動詞相当表現判定は、当初計画したja側固定語彙リストのcontains判定では実在の正当な模範文言（「作業を切り上げる」等）を網羅できないことが実装時に判明したため、日本語は動詞語尾（godan/ichidan終止形・て/で形・「ください」）へのsuffix一致、英語は拡充語彙リストへのcontains一致（`startsWith`不使用）というロケール非依存のOR判定へ設計を修正した。
5. `AiMetrics`へ`sanityRejectCount`／`lastSanityRejectReason`を、`AiResult.Fallback`へ`metrics: AiMetrics?`を追加する（いずれも既定値付き追加、既存呼び出し元は無改修。L2がPrimary/Retryの各attemptでrejectした回数・直近のreject理由をL5として記録し、Phase 12のAnalytics定量化基盤への先行対応とする）。
6. `generateRecovery`の実装は`LocalAiGateway`の既存`inferenceMutex.withLock { }`（T-GW-15、`generatePlan`と同一パターン）を必ず経由する。既存Mutexの保護範囲は変更しない。**実装調査による経緯の記録**: 計画段階のレビュー対応（Gemini G3指摘A-6）で、当初`ai/adapter/LiteRtLmLocalLanguageModel.kt`の`engineLifecycleMutex`（Engine取得のみを保護し実推論呼び出しはmutex外）のみを確認し「Plan/Recovery推論は直列化されていない」と誤って結論したが、`AiFallbackReasonTest.kt`のコメントを手がかりに`LocalAiGateway.kt`自身が別途保持する`inferenceMutex`（`checkInstalledModel`〜`invokeModel`の全区間を包む）を発見し、当初の結論を訂正した（計画書§8・§13「実装調査による追加発見」参照）。
7. §32 option 3「Prepare a delay message」はPhase 10以降へ再送りする（§88「予定を今やる一つの行動に変えることに直接寄与するか」への適合度が低く、「対外連絡」という質的に異なるリスク領域を本フェーズの主目的と混在させないため）。L1（few-shot再設計）・P1〜P5（性能施策）はPhase 9.5へ分離する（Phase 9のA54実機受け入れ時にL1-a等の実機挙動を実装を伴わない観察のみ行うことは許可する）。R1bはL1-aのカテゴリ推定基盤を要するためPhase 9.5送りとする。`RecoveryPromptBuilder`の入力からstart_time（出発予定時刻の文脈）は引き続き除く（現行案どおり維持、Phase 9.5で「間に合わせる」文脈の要否を再検討する余地は残す）。

**代替案と却下理由**:

| 代替案 | 却下理由 |
|---|---|
| `ContentSanityChecker`の既存digit-ban（捏造検出）ルールを緩和する（Gemini G2指摘） | §15「時刻演算をLLMに渡さない」の直接実装であり正しい既存ルール。緩和すると数字・時刻の捏造をexplanationが通過し得るため不採用 |
| バイナリ互換性への配慮として`AIRecoveryResponse`等の契約変更を避ける（Gemini G8指摘） | 本アプリは単一モジュール・単一APKで配布され、これらの型は外部公開APIではないため該当しない懸念（外部SDK等での懸念であり本プロジェクトの構成には非該当） |
| AI応答完了までRecovery画面の描画自体を待機する（先に見せない、Gemini G1指摘） | §19「Local AIはEnhancement、SPOFにしない」に違反する。Basic即時表示は本計画の全編を通じた不変の前提であり、これを崩す提案は不採用 |
| R1（few-shotエコー検出）を単一ルールのまま実装する | 「本物の歯科検診の予定に対しAIが正当に出力したケース」まで模範例のmodelSteps出力と近似一致して誤って弾いてしまう（両レビュー一致の最重要指摘）。R1a（模範例タイトルのecho、常時reject）／R1b（modelStepsのdisplay_text echo、カテゴリ一致判定必須、Phase 9.5送り）へ分割した |
| `RecoverySchemaValidator`のJSONクレンジング処理を`SchemaValidator`と共通化する（Gemini G9指摘） | 実装調査の結果、`SchemaValidator`に「前置き文/コードフェンスを剥がす」処理は実在しない（非寛容パース方針が既存の正しい設計）ことが判明。共通処理の再利用ではなく同一の非寛容パース方針の独立採用とした |
| pairing検証（`RecoverySchemaValidator`）をBest-Effort部分適用にする | Recoveryは最大3件のみで独立レスポンス分割の複雑化に見合わない。All-or-Nothing（集合完全一致）を採用し、L4（`overlay`のper-option部分適用が可能な既存構造）が土台を持つことのみ記載し実装は本フェーズでは行わない（再検討トリガー参照） |

**影響範囲**: 新設4ファイル（`ai/LocalAiRecoveryContextualizer.kt`・`ai/prompt/RecoveryPromptBuilder.kt`・`ai/schema/RecoveryJsonSchema.kt`・`ai/schema/RecoverySchemaValidator.kt`）＋対応`Test.kt`各4本。変更: `ai/AIRecoveryResponse.kt`（フィールド構成縮小）・`ai/LocalLanguageModel.kt`（`generateRecovery`シグネチャ変更）・`ai/LocalAiGateway.kt`（`generateRecovery`本実装・L2/L5配線・`AiResult.Fallback.metrics`／`AiMetrics`拡張・`invokeModel`／`invokeRecoveryModel`の`invokeModelCall`共有化）・`ai/AiFallbackReason.kt`（`NOT_IMPLEMENTED_IN_PHASE7`削除）・`ai/schema/ContentSanityChecker.kt`（L2 R1a/R2追加・`checkRecovery`新設・`rejectReason`付き`Invalid`）・`ai/adapter/LiteRtLmLocalLanguageModel.kt`（`generateRecovery`実装）・`ai/prompt/PlanPromptBuilder.kt`（`fewShotEventTitles`アクセサ新設）・`features/recovery/RecoveryOptionText.kt`（`aiExplanation`引数）・`features/recovery/RecoveryViewModel.kt`（`aiRecoveryContextualizer`注入・世代トークン方式のstale-write防御）・`features/recovery/RecoveryScreen.kt`（呼び出し引数追加・`minLines=2`）・`di/AppContainer.kt`（`localAiRecoveryContextualizer`新設・`RecoveryViewModel`初期化子配線）。既存`recovery/BasicRecoveryEngine.kt`・`RecoveryPlanApplier.kt`・`LatenessDetector.kt`・`BasicRecoveryDefaults.kt`・`domain/model/RecoveryOption.kt`／`RecoveryContext.kt`／`RecoveryPlan.kt`・`ai/model/`配下全体は無改修（計画書§5「非変更」）。

**検証方法**: 3コミット構成（コミット1`dca7150`＝AI基盤、コミット2`81eec58`＝L2/L3/L5品質防御ハーネス、コミット3`3c0ce4c`＝UI配線）のいずれも段階完了時点で`:app:testDebugUnitTest`全件Green・`:app:lintDebug` error 0を維持した。最終状態（コミット3時点）は`:app:testDebugUnitTest` tests=719／skipped=1／failures=0／errors=0（`app/build/test-results/**/*.xml`集計で確認、既存713件は無傷）、`:app:lintDebug` error=0／warning=23（`--rerun-tasks`で再確認、Phase 8.5終了時点から不変）。A54実機（SCG21）での受け入れ手順（計画書§10）を2026-08-11 21:36〜21:40に実施し合格（計画書§14「A54実機受け入れ結果」参照、スクリーンショット5枚`docs/evidence/screenshots/phase9/`）: Recovery画面のexplanationがBasic静的文言からAI生成文言へ約25秒以内（ウォームエンジン）に差し替わり、タイトル・ETA・カード寸法（`minLines=2`）は不変のまま構造保証とレイアウト安定swapが実機成立した。プラン側では同カテゴリの正当な文言（「歯科検診」の予定に「歯科検診を受ける場所を確認する」）がR1a/R2の防御を誤爆せず通過した。`selected_model_id=auto`のままPlan/Recovery両生成が完走し、Recovery専用の解決経路が存在しないこと（T-P9-27）を実機確認した。0.6B規模ゆえのexplanation文意のぎこちなさ（「必ずしも行う必要がある」）は正直に記録し、品質防御ハーネス自体の通過判定（非エコー・動詞相当表現あり・数字/時刻捏造なし）は正当であることと区別した。

**再検討トリガー**: L5実測（`sanityRejectCount`／`lastSanityRejectReason`）で、pairing不一致（`RecoverySchemaValidator`のAll-or-Nothing全体reject）に起因するFallback率が高いことが判明した場合、Best-Effort部分適用（決定・代替案表参照）への緩和を検討する。Plan/Recovery同時発火時の`inferenceMutex`直列化による遅延がPhase 9.5のP1（Engine事前ウォームアップ）等で実機体感上問題になった場合は、直列化方式自体を再評価する。AI生成explanationの文意の自然さ（本ADR「検証方法」記載のぎこちなさ）がPhase 9.5のL1（few-shot再設計）で改善しない場合、モデル規模自体（Qwen3-0.6B→より大きいモデルへの既定変更）の再検討をPhase 12の定量データを踏まえて行う。

### ADR-0064: 計測駆動でPhase 9.5の性能・品質施策を確定する（Recovery pairing交差一致緩和・warmゲート二層修正、few-shotカテゴリ条件選択は不採用、Engineウォームアップ採用、Recoveryプロファイル分離は構造縮退、GPUバックエンドはNO_GO確定、マスク模範例は縮退）（Phase 9.5、仕様§73フォローアップ）

- 日付: 2026-08-12 ／ ステータス: 採用（`docs/plans/phase9.5-performance-quality.md`の敵対的レビュー反映後に実装着手、M〜PR-2の全マイルストーン完了・実機検証（A54実機を含む）をもって確定） ／ 決定者: domain-implementer（各マイルストーンのStep 4実装完了・実機検証結果をオーケストレーターが確認した時点の判断） ／ 起案agent: domain-implementer ／ 関連仕様§: §73「Phase 9・Local AI Recovery」性能・品質フォローアップ、§57「性能指標」、§11.1〜§11.3実機実測系、ADR-0056（サンプリング設定）、ADR-0057（few-shot拡張・`estimateMaxNumTokens`）、ADR-0059（P7-C8モデル比較・`defaultProfilePeakRamBytes`実機実測）、ADR-0062（`ModelSelector`自動選択）、ADR-0063（Phase 9 Local AI Recovery、本フェーズの直接の前提）

- **ADR番号の付番根拠**: 起票直前に`grep -n "^### ADR-" DECISIONS.md | tail -3`を再実行し、最新確定ADRがADR-0063（本書1632行、Phase 9）であることを確認した。計画書§9の起案時点の前提（「最新確定ADRはADR-0063、本計画の決定はADR-0064（暫定）」）と一致しており、齟齬はない。

**背景**: Phase 9はA54実機受け入れに合格したが（`docs/plans/phase9-recovery-ai.md`§14）、その実機確認は単発試行にとどまっていた。Phase 9.5は「①ベースライン実測→②改善実装→③A/B再実測で効果を数値確認する」という計測駆動の方法論（計画書§0・§4）を採用し、M（ベースライン30試行計測）を皮切りに、実測が示す欠陥・仮説の実測検証・実機のGPU/メモリ制約の実地確認という3種の情報源から、本ADRが記録する7件の決定に到達した。単なる「実装して終わり」ではなく、**実測がなければ気づけなかった欠陥（F-4・F-5）／実測でなければ確信できなかった不採用（F-1）／実測すら不要だった構造的却下（F-3）／実機固有のハードウェア制約による確定（PR-1）／実測結果を根拠にした未実装判断（PR-2）**という、性質の異なる複数の意思決定パターンが1フェーズに混在した点が特徴である（計画書§14各節に個別の実測記録、§0に結論サマリ）。

**決定**:
1. **F-4（Recovery pairing検証の交差一致緩和）**: M実測でRecovery全15試行が`RecoverySchemaValidator`のpairing検証（`expectedSemanticActions`との完全一致）不成立により一貫して`SCHEMA_INVALID`でFallbackすると判明し、計画書§9の再検討トリガー「pairing不一致に起因するFallback率が高い場合はBest-Effort部分適用（交差一致）への緩和を検討する」の発動条件が実測で成立した（計画書§14「重大発見①」）。pairing検証を完全一致から**交差一致**へ緩和し、`expectedSemanticActions`との交差が1件以上あれば交差分のみを`AIRecoveryResponse.options`として採用し`Valid`とする（交差0件は引き続き`Invalid`、重複検出は緩和しない）。あわせて`RecoveryPromptBuilder.buildSystemInstruction`のルール1文言を強化した（計画書§3.9）。
2. **F-5（Planウォームゲート自爆の二層修正）・F-5b（既定値配線ギャップ修正）**: M実測でPlanの2回目以降呼び出し（ロード済みEngine再利用時）にもOOM事前ガードが「これから新規ロードする」前提の閾値を再適用し自爆的にFallbackする欠陥を発見した（計画書§14「重大発見②」、5バッチ中2バッチで再現）。`ModelSelectorImpl.select()`自身のavailMemフィルタと`LocalAiGateway`層の両方（二層構成）を修正したが、Green後の実機A/BでF-5が未発動と判明し、`LocalAiGateway`コンストラクタの既定値`modelSelector`に`engineLoadStateSource`が自動配線されていなかった統合ギャップをF-5bで是正した（計画書§3.10、§13「Red検収時点の追加訂正」「Green後A/B実測時点の追加訂正」）。実機A/B（コミット08de59a）でRecovery完全蘇生（M時点0/15 Success→F-5b後3/3 Success、二重試行込み27.5〜29.7秒→単発成功約10秒）を確認した（計画書§14「A/B再実測（F-4/F-5/F-5b後）」）。
3. **F-1／F-1b（few-shotイベントカテゴリ条件選択）は不採用・ロールバック確定（F-1c）**: プレフィル短縮とエコー素材根絶を狙い`EventCategoryClassifier`（ja/en別辞書）によるカテゴリ条件付きfew-shot選択をPlan限定で実装したが、実機A/B実測1回目（コミット09f4d99、各カテゴリseed1件）で`TITLE_COPY`3/3 Fallback、プールを各カテゴリ2件へ増強した2回目（コミット12a14d6）でも`MIN_QUALITY`3/3 Fallbackと、失敗理由を変えながら2回連続で有意な品質退行を実測した（計画書§14「A/B再実測（F-1）」「A/B判定（F-1b）」）。計測駆動フェーズの撤退基準（2連続陰性）が発動したため、`eventTitle`引数によるカテゴリ条件選択の本番配線を不採用としロールバックした（コミット608a478）。`EventCategoryClassifier`・カテゴリ絞り込みロジック・増強済みseedプール・関連テスト（T-P95-1〜12）は削除せず、KDocに配線ロールバック済みと明記のうえドーマント基盤（Phase 12実験材料）として残置する（計画書§3.2「結論」）。
4. **F-2（Engineウォームアップ）採用・F-2b（ヘッドルーム再調整）**: `LocalAiGateway.warmUp()`を新設し、既存の事前ガード列（`aiEnabled`→Tier→ABI→`resolveInstalledEntry`）に加え強化availMemガードを通過した場合のみ、トップ画面（`EventSelection`）入場時に1回・in-flight時はスキップしてEngineを先行ロードする（features層は`LocalAiGateway`のみを参照する既存層規律を維持、計画書§3.3敵対的レビューA-5・A-7、コミット3dcb207）。当初、強化ガードのマージン`WARM_UP_EXTRA_HEADROOM_BYTES`を通常ガード（512MB）より厳しい+1GiBとしたが（同A-6）、A54実機検証でウォームアップが不発（PSS 108,062KBのままモデル非常駐）と判明し、原因は実際に解決されたQwen3-0.6Bの`defaultProfilePeakRamBytes`（1.25GiB）に対する旧要求2.25GiBへ起動時availMem（2.28GiB）が僅差で近接し、`warmUp()`実評価時点にはアプリ自身の起動消費で閾値割れしていたことにあった（+1GiBが6GB端末で常閉の門になっていた）。`WARM_UP_EXTRA_HEADROOM_BYTES`を通常ガードと同値の**512MB**へ再調整し（F-2b、コミットf08859a）、実機再検証でPSSが526,475KBまで増加しウォームアップ発動・エンジン常駐を確認した（計画書§3.3、§14「F-2b実機再検証」）。
5. **F-3（Recovery用maxNumTokensプロファイル縮小）は構造分析による縮退（descope）確定**: `LiteRtLmLocalLanguageModel`のEngineがプロセス内シングルトン（R-7）であり`maxNumTokens`がコンストラクタレベルの単一値としてPlan/Recovery両方に共有される構造上、Recovery専用プロファイルのEngineへの実配線は(a) Plan/Recoveryの大きい方を採用する統合は実質現状維持で無益、(b) プロファイル不一致のたびに再ロードする統合はPlan⇄Recovery往復でスラッシングし時間クリティカルなRecovery応答に約1.4秒のロード遅延を差し込む最悪UX、のいずれも採用に値しないと判断し、**実機計測を1回も実施せず構造分析だけで**不成立と裁定した（計画書§3.4「F-3裁定」、§14「F-3裁定（計測以前に構造が答えを出したケース）」）。見積もり関数`RecoveryPromptBuilder.estimateMaxNumTokens`自体はGreen実装・テスト済み（T-P95-58〜62）のドーマント設計文書として残す。`LiteRtLmLocalLanguageModel`側の配線用スタブは将来性がないため削除した。
6. **PR-1（GPUバックエンド可否）は実機検証によりNO_GO確定**: litertlm-android 0.15.0のAAR実体（Gradleキャッシュの`-api.jar`）を`javap -p`で直接調査し、`Backend.GPU()`（引数なしコンストラクタ）の存在と`EngineConfig.backend`パラメータへそのまま渡せることを確認した（API存在＝GO）ため、CPU/GPUを完全同一の`EngineConfig`・`ConversationConfig`・data messageで比較する`GpuBackendProbeTest`を新設した（コミット25f069e）。A54実機で実行した結果、GPU側はEngine初期化（`modelLoadMs=6286`）までは成功したものの、初回推論（`sendMessage`）が`LiteRtLmJniException: Can not find OpenCL library on this device`で失敗した（CPU側はTTFT約2.67秒・約9.71tok/sで完走、`build/agent-logs/phase9.5-gpu-probe-logcat.log`）。A54（SCG21）はMali-G68 GPUを搭載するがOSがOpenCLライブラリをアプリへ公開しておらず、LiteRT-LMのGPUバックエンドは本実機では実行不能——**GPU=NO_GO確定**。本採用は行わない（計画書§3.5、§14「PR-1実機検証結果」）。
7. **PR-2（マスク模範例・カテゴリ条件付きecho検出）は縮退（descope）確定**: L1-b（few-shotの`userTurn`を`[EVENT_TITLE]`等へマスクする設計）・R1b（`ContentSanityChecker`へのカテゴリ条件付きecho検出追加）について、①本フェーズを通じた全計測（M・各A/B・GPU probe含む）でecho発生が0件（既存L2防御R1aで実害なし）、②L1-bの模範例構成変更はF-1が実証した「構成変更が0.6B品質を崩しうる」という同型リスクを便益不明のまま負う、③R1bが依存するカテゴリ条件付け（F-1）はA/B実測2回で陰性・ロールバック確定済み、の3点によりプローブを実施せず縮退すると裁定した。Phase 12（品質定量化環境整備後）での再評価へ申し送る（計画書§3.6、§14「PR-2裁定」）。

**代替案と却下理由**:

| 代替案 | 却下理由 |
|---|---|
| F-2bのヘッドルームをゼロ（通常ガードと完全同一視し定数自体を廃止）にする | ウォームアップ固有の関心事（不要な先行ロードのコスト）を表現する`WARM_UP_EXTRA_HEADROOM_BYTES`という定数自体は維持し値のみ実測に基づき是正する方が、変更の意図（「同一メモリを数秒早く確保するだけの行為に独自の倍マージンを課す合理性がない」という判断の経緯）を追跡しやすいと判断した |
| F-3: Engineを複数プロファイル同時保持可能に拡張しPlan/Recoveryを共存させる | 現行LiteRT-LM APIはシングルトンEngine・単一プロファイル構造であり、複数プロファイル同時保持や再ロードなしでのコンテキスト長切替はKVキャッシュ機構（計画書§3.8「記録のみ」定点観測）を要する将来のランタイム機能。過剰設計を避け、見積もり関数のみドーマント残置とした |
| PR-1: GPU NO_GO判明後もモデルの量子化方式を変えて再挑戦する | 失敗原因はモデル側の量子化方式（INT4 block-32）ではなく、A54のOS層がOpenCLライブラリをアプリへ公開していないという環境側の構造的制約である。ハードウェア（Mali-G68）は実在するがOSの通り道が閉じているため、モデル側の工夫では解決できない根本原因と判断した |
| PR-2: L1-bを先に実装してから効果測定する | L1-bの模範例構成変更（few-shotの`userTurn`をマスクへ差し替える）は、F-1が実機A/Bで実証した「模範例構成の変更が0.6B品質を崩しうる」というリスクと同型である。実装コストをかけて確認するまでもなく、F-1の実測結果が既に十分な判断材料を提供していると判断し、プローブすら実施せず縮退した |
| F-1: ドーマント化ではなくコードから完全削除する | `EventCategoryClassifier`・カテゴリ絞り込みロジック・増強済みseedプールはPhase 12（品質定量化環境整備後、より大きいモデルでの再評価）における実験材料としての再利用価値があるため、削除せずKDocに「本番配線はF-1 A/B負判定によりロールバック済み」と明記のうえ残置した |

**影響範囲**: 新設ファイル: `ai/EngineLoadStateSource.kt`（F-4/F-5）・`ai/EngineWarmable.kt`（F-2）・`ai/prompt/EventCategoryClassifier.kt`（F-1、ドーマント残置）・`app/src/androidTest/.../probe/GpuBackendProbeTest.kt`（PR-1）＋対応`Test.kt`。変更（延べ、Phase 9.5全マイルストーン）: `ai/LocalAiGateway.kt`（`warmUp()`新設・`generateRecovery`のpairing/OOMガード修正・`WARM_UP_EXTRA_HEADROOM_BYTES`調整）・`ai/adapter/LiteRtLmLocalLanguageModel.kt`（`EngineWarmable`実装・F-1配線のロールバック・F-3スタブ削除。Phase 9.5 RF-1で`generatePlan`／`generateRecovery`・`buildConversationConfig`系・`buildDataMessage`系の重複を`runInference`／`toConversationConfig`／`toMessages`／`withConcisenessConstraintIfNeeded`へ統合、コミット0f423f4、詳細は同コミット報告のメソッド対応表参照）・`ai/model/ModelSelector.kt`（F-4/F-5二層目）・`ai/prompt/RecoveryPromptBuilder.kt`（F-4ルール強化・F-3見積もり関数追加）・`ai/prompt/PlanPromptBuilder.kt`（F-1カテゴリ条件選択、ロールバック後もドーマント残置）・`ai/schema/RecoverySchemaValidator.kt`（F-4交差一致緩和）・`di/AppContainer.kt`（F-2/F-5配線）・`features/eventselection/EventSelectionViewModel.kt`（F-2ウォームアップ実配線）。既存`recovery/`配下・`domain/model/`配下・`ai/schema/ContentSanityChecker.kt`は無改修。計画書`docs/plans/phase9.5-performance-quality.md`を全マイルストーンで継続更新（§3・§7・§12・§13・§14）。

**検証方法**: 各マイルストーンのStep 4完了時点で`:app:testDebugUnitTest`全件Green・`:app:lintDebug` error 0を維持した。最終状態（RF-1・コミット0f423f4時点）は`:app:testDebugUnitTest` tests=757／skipped=1／failures=0／errors=0（`app/build/test-results/**/*.xml`集計で確認）、`:app:lintDebug` error=0／warning=23（`--rerun-tasks`で再確認）。実機検証はA54（SCG21）で複数回実施した: Mベースライン30試行計測（`build/agent-logs/phase9.5-baseline-logcat.log`）、F-4/F-5/F-5b後のA/B（3/3 Success、`phase9.5-f45-ab-logcat.log`）、F-1のA/B（3/3 Fallback、`phase9.5-f1-ab-logcat.log`）、F-1bのA/B（3/3 Fallback、`phase9.5-f1b-ab-logcat.log`）、F-2b再検証（PSS108,062KB→526,475KB、ウォームアップ発動確認）、PR-1のGPUバックエンドプローブ（`phase9.5-gpu-probe-logcat.log`、CPU完走・GPU=NO_GO）。`ai/adapter/LiteRtLmLocalLanguageModel.kt`はJVMテストで一切検証できない（class file version不一致）ため、同ファイルに変更を加えたF-1/F-1c/F-2/F-3/RF-1はいずれも実機androidTest（計画書§10実機受け入れ手順）による最終確認を経てクローズした。RF-1（重複統合リファクタ）自体は変更を「挙動同値の機械的統合」に限定し、統合前後のメソッド構造対応表をコミット報告に含めることで、JVMで検証できない範囲の変更リスクを最小化した。

**再検討トリガー**: (a) LiteRT-LMの将来リリースでKVキャッシュ／プレフィックスキャッシュAPI（計画書§3.8定点観測）が追加された場合、F-3が却下したプロファイル分離を再評価する。(b) `onTrimMemory`連動でのウォーム済みEngineの能動的アンロード（F-2/F-2bとも本フェーズ非実装）を実装した場合、F-2bが確定した512MBというヘッドルーム値の妥当性（遊休常駐コストの性質が変わるため）を再評価する。(c) litertlm-androidの将来リリースでOpenCL以外のGPU delegate経路（Vulkan等）が追加された場合、またはAndroid OS側でOpenCLライブラリの公開範囲が変わった場合、PR-1のプローブ（`GpuBackendProbeTest`は削除せず残置）を再実行する。(d) Phase 12の品質定量化環境が整備された場合、PR-2（L1-b／R1b）およびF-1（より大きいモデルでのカテゴリ条件選択再評価）の両方を再評価する。

### ADR-0065: 行動ログ（`BehaviorEventEntity`）とPersonal Profile永続化（`RoomAnalyticsStore`）を導入し、`BasicPlanningEngine`のハードコード既定値を実績中央値へ置き換える（Phase 10、仕様§74・ADR-0049決定5への回答）

- 日付: 2026-08-12 ／ ステータス: 採用（`docs/plans/phase10-behavior-log-profile.md`の敵対的レビュー反映後に実装着手、C1〜C4の全コミット完了・Step 4実装完了時点で確定。実機受け入れ（計画書§10）はオーケストレーターが本ADR起票後に実施） ／ 決定者: domain-implementer（各コミットのStep 4実装完了・全806件Green＋lint 0をオーケストレーターが確認した時点の判断） ／ 起案agent: domain-implementer ／ 関連仕様§: §74「Phase 10」（Personal Profile・履歴保存・次回Planへ反映）・§52「Personal Execution Profile」（Kotlin型定義）・§22/§23（Personal Profile構想）・§53「Analytics / Test Log」・§54「時刻ログ」・§55/§56（KPI定義）・§10「Local-first AI」・§58「Privacy」・§59「Local Data」・§60「Telemetry」・§32「Recovery Option」・§61「MVPに入れない機能」・§34「ユーザー最終決定」・§88「Developer UX Principle」、ADR-0025（`ExecutionScheduleStore`のRoom移行「経路として維持」）、ADR-0049決定5（`AnalyticsStore`をPhase 10/12で導入する旨の再検討トリガー、本ADRが正式な回答）

- **ADR番号の付番根拠**: 起票直前に`grep -n "^### ADR-" DECISIONS.md | tail -3`を再実行し、最新確定ADRがADR-0064（本書1666行、Phase 9.5）であることを確認した。計画書§9は起案時点で特定の前提ADR番号を明記していない（「起票直前に再実行し確認したうえで正式起票する」という手順のみを規定）ため、本ADRの番号は上記grep結果と齟齬なくADR-0065として確定する。

**背景**: 第1弾最終採点（97.6/100、`docs/evidence/scorecard-final-goal.md`）における残存5件のうちC-18「行動ログ未実装」は第1弾スコープ唯一の直接の未達項目であり、同スコアカードは「Phase 10（Personal Profile）の前提データでもあるため、小型フォローアップまたはPhase 10冒頭での実装を推奨」と明記していた。`domain/model/PersonalExecutionProfile.kt`はPhase 8.5 C2で既にscaffold実装済みで、KDocに「永続化はRoomで行う（Phase 10、仕様§74）」と明記されている。`BasicPlanningEngine.kt`は`PlanningContext.profile`（nullable）の6フィールド中2つ（`averageTransitionDuration`／`averagePreparationDuration`）のみを参照し、profile不在時は`BasicPlanningDefaults`のハードコード値（移動5分・準備15分）へフォールバックする——このフォールバックは「Phase 10まで」の暫定値として最初から明示的に設計されていた。ADR-0049決定5は「Analytics collaboratorは追加しない。T-GW-14はPhase 10（`AnalyticsStore`導入）／Phase 12（Analytics実装）とともに実装する」と述べ、本フェーズが導入すべきクラス名を`AnalyticsStore`と明示していた。本フェーズはこれら3つの前提（C-18・Profile scaffold・ADR-0049トリガー）を統合し、Room（本プロジェクト初のDB）導入からBasicPlanningEngineへの実績値反映までを4コミット（C1〜C4）で完了させた。

**決定**:
1. **Room 2.8.4 + KSP 2.3.11採用（本プロジェクト初のDB、C1）**: `androidx.room:room-runtime`／`room-compiler`（KSP、kapt不使用）を導入した。DB名`behavior_log.db`、`exportSchema = true`とし、スキーマJSONを`app/schemas/`へコミットする運用とした（§12確認事項5）。
2. **行動ログ最小5種スキーマ（`BehaviorEventEntity`、C1/C2）**: 仕様§53が定める19種の広い語彙のうち、コーディネーター指示どおり最小集合（`STEP_DONE`／`STEP_SKIPPED`／`DELAY_DETECTED`／`RECOVERY_SELECTED`／`AI_WORDING_OUTCOME`）のみを実装した（§12確認事項8）。`domain`列（`"plan"`／`"recovery"`の2値）を新設し、`AI_WORDING_OUTCOME`をPlan側（`PlanReviewViewModel`）・Recovery側（`RecoveryViewModel`）の両方で記録する（レビューCRITICAL、Phase 12比較実験の主データはPlan側）。`stepType`列（C2で追加）により`STEP_DONE`の実績を`ExecutionStepType`単位で区別する。`DELAY_DETECTED`は当初案（NavHost層のフック）から`RecoveryViewModel.init`へ設計変更した（レビュー§13 No.4、ナビゲーション層への層違反とComposable再コンポジションでの多重記録リスクを回避）。
3. **タイトル生文非保存（C1）**: `eventCategory: String`は`EventCategoryClassifier.classify(title, locale)`の戻り値（カテゴリのみ）をその場で計算し格納し、イベントタイトル自体は一切保持しない（§12確認事項2、§10「Behavioral Historyを外部LLMへ送らない」の機微データ列挙を継承した設計）。pinning回帰テスト（T-P10-9）で恒久的に固定した。
4. **バックアップ除外（WAL/SHM含む3ファイル、C1）**: Roomのjournal_mode既定（WAL）がDB本体に加え生成するサイドカーファイル`behavior_log.db-wal`／`behavior_log.db-shm`を含む3ファイルを、新旧両方式（`android:dataExtractionRules`〔API 31+〕・`android:fullBackupContent`〔API 26-30〕）双方の除外ルールへ明記した（レビューCRITICAL・§13 No.2、1ファイルのみの除外はWAL/SHM経由のデータ漏出を防げないため）。ソーススキャン型pinningテスト（T-P10-9b）で両XMLの3ファイル列挙を固定した。
5. **保持期間: 直近180日 or 直近500件の小さい方（C3）**: 仕様に規定がないためコーディネーター確認事項として確定した値（§12確認事項3）。集計クエリ自体が独立した`timestamp`述語（`WHERE timestamp > (現在時刻 − 集計窓ミリ秒) ... LIMIT 500`）でこの窓を実現し、別途の物理ローテーション処理の実行有無に依存しない設計とした（レビュー§13 No.9b）。
6. **Personal Profile中央値集計とBasicPlanningEngineの2フィールド反映（C3）**: `averageTransitionDuration`／`averagePreparationDuration`の2フィールドに厳格限定して算出する（レビューCRITICAL・§13 No.3）。フィールド名は`average*`だが実装は中央値を採用した——サンプル数が少ない初期段階で外れ値（渋滞・忘れ物取り戻り等）に平均が引っ張られることを避けるための意図的な命名との乖離であり、KDocに明記した。`BasicPlanningEngine.kt`は無改修で通過した（既存の`profile?.averageTransitionDuration ?: BasicPlanningDefaults.TRANSITION`等のフォールバックが本フェーズの要件と完全一致していたため、実装前の読解で確認）。イベント記録の都度、非同期で該当カテゴリのみ再集計しupsertする（ユーザー操作をブロックしない、Phase 9.5で確立した「補助データはブロッキングしない」原則を継承）。
7. **計測不能3フィールドは`Duration.ZERO`プレースホルダ（C1→C3で確定）**: GPS/到着検知の実装が本アプリに存在しないため`averageResponseDelay`／`averageDepartureDelay`／`preferredArrivalBuffer`の3フィールドは本フェーズでは算出不能。エンティティ上はnull許容で保持し、`RoomAnalyticsStore.getProfile()`がドメイン型へ変換する時点でこれら3フィールド（および行が存在するが該当メトリクスのサンプルがない場合の残り2フィールド）を一律`Duration.ZERO`へ変換する。カテゴリ自体の行が存在しない場合のみ`PersonalExecutionProfile`全体を`null`とする（C1メモの設計テンションをC3で解消、KDoc明記）。
8. **全削除導線（Settings、C4）**: Settings画面に「行動ログを削除」ボタン＋確認ダイアログ（破壊的操作ガード必須）を新設した。確認ダイアログの確定操作のみが`AnalyticsStore.clearAll()`を呼ぶ構造的に唯一の経路であり（誤タップ防止、T-P10-18）、削除結果（成功/失敗）はサイレント化せずUIへ明示する（§8「削除処理自体の失敗は握り潰さずUIへ結果を返す」）。`RoomAnalyticsStore`内の単一`Mutex`が通常の記録・Profile集計・`clearAll()`の全操作を直列化し、`clearAll()`実行中の並行書き込みによる競合を防ぐ（レビュー§13 No.6・Gemini G6）。この直列化はCompletableDeferredシグナル方式（T-P95-55と同型）による決定的テスト（T-P10-16b）とミューテーション検査（mutex除去でRed化を確認）で実効性を証明した。
9. **スコープ外の確定**: (a) Profileのプロンプト注入（AI個人化）は見送り、Phase 11/12へ（§12確認事項4。理由: 完了直後はProfileデータがほぼ空でコールドスタート、Phase 9.5のF-1が実証した「実機未検証のプロンプト変更は0.6B品質を崩しうる」教訓の直接適用、Phase 12比較実験基盤が整うまで計測なしに投入すべきでない）。(b) delay message（§32 option 3）は3度目の再送り（§12確認事項1。§61自動送信禁止・§88目的適合度・「対外連絡」という質的に異なるリスク領域が理由、Phase 6→9→10で不変）。(c) `ExecutionScheduleStore`のRoom移行は見送り（§12確認事項6。ADR-0025は「移行する場合の経路」を残しただけで移行を義務付けていない。現状PII-zero・SharedPreferencesで機能上の問題なし）。(d) DBスキーマ不整合時の`fallbackToDestructiveMigration()`は既定で有効化しない（§12確認事項7。データ消失を伴う自動復旧よりも、`AppContainer`の`by lazy`＋try/catch→null防御による初期化失敗時のno-op縮退〔行動ログ機能のみ無効化、アプリ本体は継続動作〕を優先）。
10. **ADR-0049決定5の再検討トリガーへの回答**: 同ADRの再検討トリガー「Phase 10の`AnalyticsStore`設計時に本ADRを参照し、コラボレータの正式な注入方式を設計すること」に対し、`AnalyticsStore`インターフェース（意味論的5メソッド＋`getProfile`／`clearAll`）＋`RoomAnalyticsStore`実装を新設し、`AppContainer`の`by lazy`＋try/catch→null（既存`LinkageError`防御と同型）で保護したうえで4つのViewModel（`ExecutionViewModel`／`RecoveryViewModel`／`PlanReviewViewModel`／`SettingsViewModel`）へ末尾・既定`null`の後方互換パラメータとして注入する設計で正式に回答した。`AiMetrics`（Phase 9新設、呼び出し単位のインメモリ診断構造体）とは別概念であることを踏襲した。

**代替案と却下理由**:

| 代替案 | 却下理由 |
|---|---|
| `AnalyticsStore.record(event: BehaviorEventEntity)`という単一の汎用メソッドを公開する（C1当初案） | features層がRoom型（`BehaviorEventEntity`）を直接構築する必要が生じ層違反となる。C2で`recordStepDone`等の意味論的5メソッド＋`AnalyticsDomain` enumへ再設計した |
| `DELAY_DETECTED`のログをNavHost（`ActionStarterNavHost`）のComposable内、`LatenessDetector.evaluate`の判定分岐に直接配置する（コーディネーター原案） | ナビゲーション層への層違反であり、Composable再コンポジションによる多重記録リスクを抱える（レビュー§13 No.4、Gemini G3／オーケストレーターAD11の両者一致）。`RecoveryViewModel.init`（Recovery画面遷移＝遅延確定の唯一の経路）へ移設した |
| Personal Profile集計をaverage（算術平均）で実装する（フィールド名`average*`の字義どおり） | サンプル数が少ない初期段階（Personal Profileの本質的制約）で外れ値1件（渋滞・忘れ物取り戻り等）に平均が引っ張られることを避けるため、命名との乖離を明記したうえで中央値を採用した（レビュー§13 No.3） |
| 集計クエリの対象時間窓を、別途実行される保持期間ローテーション処理の完了状態に依存させる | ローテーション処理のタイミングやバグに集計結果自体が左右される設計となる。集計クエリ自体を独立した`timestamp`述語でフィルタし、ローテーションの実行有無と無関係に正しい窓を維持する設計とした（レビュー§13 No.9b） |
| 削除結果（成功/失敗）の表示に`ActionStarterNavHost`の`SnackbarHostState`（T-NAV-4で確立済みの既存様式）を使う | `SettingsScreen`自体には結びつかないNavHostスコープの状態であり、配線には`SettingsScreen`の§10.6疎結合契約（`uiState`＋コールバック引数のみで完結）を破りNavHost状態への依存を持ち込む必要があった。同じファイル内に既にある条件付きインラインText様式（`settings_ai_unsupported_reason`等）を採用した |
| DBスキーマ不整合時に`fallbackToDestructiveMigration()`を既定で有効化する | データ消失を伴う自動復旧であり、初期化失敗時に無言でユーザーデータを破棄するリスクを負う。`AppContainer`のno-op縮退防御（行動ログ機能のみ無効化、アプリ本体は継続動作）を優先した（§12確認事項7） |
| Personal Profileのプロンプト注入（AI個人化）を本フェーズで先行実装する | 完了直後はProfileデータがほぼ空でコールドスタートの実効性がなく、Phase 9.5のF-1（few-shotカテゴリ条件選択）が実機A/B未検証のプロンプト変更で2度連続の品質退行を出した教訓と同種のリスクを、Phase 12の比較実験基盤が整うまで計測なしに負うべきでない（§12確認事項4） |

**影響範囲**: 新設: `persistence/room/BehaviorLogDatabase.kt`／`BehaviorEventEntity.kt`／`PersonalExecutionProfileEntity.kt`／`BehaviorEventDao.kt`／`PersonalExecutionProfileDao.kt`・`analytics/AnalyticsStore.kt`／`RoomAnalyticsStore.kt`・`app/schemas/`（Room exportSchema）・`res/xml/data_extraction_rules.xml`／`backup_rules.xml`＋対応`Test.kt`多数（`RoomAnalyticsStoreTest.kt`／`RoomAnalyticsStoreProfileTest.kt`／`BehaviorLogDatabaseTest.kt`／`BackupExclusionRulesTest.kt`ほか）。変更: `AndroidManifest.xml`（バックアップ属性）・`gradle/libs.versions.toml`／`build.gradle.kts`／`app/build.gradle.kts`（Room/KSP依存）・`di/AppContainer.kt`（DB／DAO／`AnalyticsStore`配線、4ViewModel初期化子）・`features/execution/ExecutionViewModel.kt`（`STEP_DONE`）・`features/recovery/RecoveryViewModel.kt`（`DELAY_DETECTED`／`STEP_SKIPPED`／`RECOVERY_SELECTED`／`AI_WORDING_OUTCOME`）・`features/planreview/PlanReviewViewModel.kt`（`AI_WORDING_OUTCOME`・Profile非同期精緻化）・`features/settings/SettingsUiState.kt`／`SettingsViewModel.kt`／`SettingsScreen.kt`（削除導線UI）・`navigation/ActionStarterNavHost.kt`（Settings route配線）・`values(-ja)/strings.xml`（新規10キー）。既存無改修（決定確認）: `domain/model/PersonalExecutionProfile.kt`（型定義不変）・`ai/prompt/EventCategoryClassifier.kt`（Phase 9.5ドーマント実装の再利用のみ）・`recovery/RecoveryPlanApplier.kt`（純粋関数のまま）・`planning/BasicPlanningEngine.kt`（既存フォールバックが要件と完全一致）・`persistence/ExecutionScheduleStore.kt`系（§3.5により見送り）。

**検証方法**: 各コミット（C1〜C4）のStep 4完了時点で`:app:testDebugUnitTest`全件Green・`:app:lintDebug` error 0を維持した。最終状態（C4・コミット885a94d時点）は`:app:testDebugUnitTest` tests=806／skipped=1／failures=0／errors=0（`app/build/test-results/**/*.xml`集計で確認、C1の764から段階的に増加）、`:app:lintDebug` error=0／warning=23（`--rerun-tasks`で再確認、`MissingTranslation`／`ExtraTranslation`は0件）。ミューテーション検査を各コミットで実施し実効性を確認した: C1（try/catch除去でT-P10-15〔書き込み失敗no-op化〕／T-P10-17〔全削除失敗Result.failure化〕が正しくRed化）、C2（Enhancement非ブロック原則、8コールサイトno-op化で該当8テストが正しくRed化）、C3（Profile再集計トリガー無効化、7件中6件が正しくRed化・1件は設計上意図的にGreen維持）、C4（T-P10-16bのMutex直列化、`clearAll()`の`mutex.withLock`除去で決定的にRed化）。実機受け入れ（計画書§10「DBが実デバイスのファイルシステム上で正しく永続化されるか（WAL/SHM含む）」「バックアップ除外ルールが実機のバックアップ設定画面で確認できるか」「Settings削除導線が実機で動作するか」の3点、既存Plan/Recovery生成の無傷確認も含む）は本ADR起票時点で未実施のまま残る（オーケストレーターが本ADR起票後に実施）。

**再検討トリガー**: (a) Phase 12でProfileのプロンプト注入（AI個人化）を実装する際、本ADRが確定した`AnalyticsStore`のクラス設計・インターフェース形状を参照し、比較実験基盤としての注入方式を再設計する（ADR-0049決定5の再検討トリガーの直接後継）。(b) 実機受け入れでWAL/SHMサイドカーファイルの除外が実機のバックアップ設定画面で確認できない場合、両バックアップXMLの記述を再検証する。(c) Personal Profileのサンプル数が実運用で十分蓄積された後もGemma4実績値とBasic既定値の体感差がPhase 12の比較実験で確認されない場合、中央値集計方式・保持期間（180日/500件）の妥当性を再評価する。(d) T-P10-16bが検証した単一`Mutex`による直列化が、実機での高頻度書き込みシナリオ（短時間に多数のRecoveryステップが連続する等）で体感可能な遅延を生むことが実測された場合、直列化の粒度（単一Mutex vs 操作種別ごとの分割）を再評価する。

### ADR-0066: 出発画面（DepartureScreen）の3欠陥を修正する — 手入力ETA算出・行き先未解決時の権限カード抑止・戻り導線の追加

- 日付: 2026-08-12 ／ ステータス: 採用（`docs/plans/departure-screen-fixes.md`のアーキテクトレビュー合格・ユーザー承認後、Red→Green完了・全819件Green確認をもって確定。実機受け入れは本ADR起票後にオーケストレーターが実施） ／ 決定者: domain-implementer（Step 4実装完了・全819件Green＋lint 0をオーケストレーターが確認した時点の判断） ／ 起案agent: domain-implementer ／ 関連仕様§: §29「Departure Mode」・§35 Screen4「Leave」・§95「Android固有のプラットフォーム制約とリスク」4節「権限一覧表」（本書内表記「§95.4」）

- **ADR番号の付番根拠**: 起票直前に`grep -n "^### ADR-" DECISIONS.md | tail -3`を再実行し、最新確定ADRがADR-0065（本書1699行、Phase 10）であることを確認した。計画書`departure-screen-fixes.md`は起案時点で特定の前提ADR番号を明記していないため、上記grep結果と齟齬なくADR-0066として確定する。

**背景**: ユーザーが実機で発見した出発画面（DepartureScreen）の3欠陥について、オーケストレーターが実コードで根因診断を行い、domain-implementerが計画立案（起案→アーキテクトレビューPass 1→ユーザー承認）を経てRed→Green実装した。3欠陥はいずれもP3-C5（Green実装）〜再デザインサイクル2の間に生じた未完了の配線であり、新規の回帰ではない——特に欠陥①は、既存テストT-DEP2-3自身のKDocが「ETA再表示はP3-C5で結線される」と当初から明記しており、`DepartureViewModel.kt`のKDoc「F28: 反映のみを行う」という記述がこの未完了配線を恒久仕様であるかのように記録してしまっていたことを実コード確認で突き止めた。

**決定**:
1. **手入力ETA計算式は`estimatedArrival = now + manualTravelMinutes分`（現在時刻起点）**: 根拠は3点の一致——(a) `travel_time_manual_label`の文言が"Travel time (minutes)"（所要時間であり出発時刻や希望バッファではない）、(b) 既存の自動計算経路`estimateAndApplyRoute`が既に同じ形（`now.plus(estimate.duration)`）を採用している、(c) 仕様§95.4の`ACCESS_FINE_LOCATION`拒否時フォールバック行の原文「現在地起点の自動ETA計算を無効化し…Travel Timeの手動入力にフォールバック」が、手入力を自動ETA計算の正式な代替経路として規定している。`arrivalBuffer`は`eventStart − estimatedArrival`で連動し、手入力が`null`（空/不正入力）に戻った場合は両フィールドとも`null`へ戻し偽ETAを残さない。
2. **行き先未解決時は権限状態に関わらず権限カードを一切表示しない**: `DepartureScreen.kt`の両権限カード（`LocationPermissionRationaleCard`＝`NOT_REQUESTED`限定・設定を開くボタン＝`DENIED`限定）の表示条件へ`&& !isDestinationUnresolved`を追加した。アーキテクトレビューPass 1指摘（当初案は権限判定順序の是正のみで、「権限拒否＋行き先未解決」の組み合わせでDENIED設定カードが残留し的外れな誘導になる不備を検出）を反映したもの——位置情報の権限設定を開いても行き先未解決という真の問題は解決しないため。
3. **`DepartureViewModel.recalculate`の権限判定順序の是正**: `hasAnyLocationPermission()`の評価を`plan.event.locationName`の空判定より前へ移動し、行き先の解決可否に関わらず`permissionState`（GRANTED／DENIED）を常に確定させる。判定自体は同期関数（システム権限ダイアログを起動しない純粋なチェック）であるため、この並べ替えは仕様§95.4「初回利用時に要求」という取得タイミング規定と衝突しない。
4. **戻り先は`EventSelection`（`popUpTo(EventSelection){inclusive=true}`）**: `ActionStarterNavHost.kt`の実コード確認により、Departure routeへの遷移元が`ExecutionScreen.onNavigateToDeparture`（Execution完了時）の1経路のみであり、`popUpTo`指定なしの`navigate`のためExecutionはバックスタックに残存していることを確認した。単純な`popBackStack()`は完了済み（アラーム・FGS停止済み・通知キャンセル済み）のExecution画面へ戻ってしまい不適切なため、`ExecutionScreen`が既に持つ`onNavigateToEventSelection`と同型の「バックスタックを`EventSelection`まで巻き戻す」パターンを踏襲した。戻るボタン（新設`TextButton`、testTag`departure_back_button`）とシステムBack（`BackHandler`）の両方を同一のラムダ（`navigateBackToEventSelection`）へ配線し、経路によって挙動が食い違う事態を構造的に排除した。
5. **`androidx.activity.compose.BackHandler`を本アプリで初採用**: Context7（`/websites/developer_android_develop_ui_compose`）で`BackHandler(enabled: Boolean = true, onBack: () -> Unit)`の用法を確認したうえで採用した。新規外部依存の追加ではなく、既存の`activity-compose`依存（`AppContainer`・`ActionStarterNavHost`が`rememberLauncherForActivityResult`等で既に使用中の同アーティファクト）に含まれる標準API。

**代替案と却下理由**:

| 代替案 | 却下理由 |
|---|---|
| 手入力ETA計算式を`eventStart − manualTravelMinutes`（出発時刻起点）とする | UI文言「Travel time (minutes)」は所要時間の入力であり出発時刻や希望バッファの入力ではない。既存の自動計算経路・仕様§95.4のいずれとも整合しない |
| 手入力ETAに上限バリデーション（例: 0〜999分）を追加する | 既存`estimateAndApplyRoute`もRouting APIの返す実測値に上限チェックを課していないため、手入力のみ特別扱いしない設計方針を踏襲した。`TravelTimeInput`の数値パース（`filter { it.isDigit() }`）が構造的に負値を生成しないことを実コードで確認済みのため、`coerceAtLeast(0)`等のクランプも不要と判断した |
| Departure画面からの戻り操作を単純な`popBackStack()`とする | Departure routeへの遷移元がExecution完了の1経路のみであり、`popUpTo`未指定のためExecutionがバックスタックに残存している。単純な戻る操作は完了済み（アラーム・FGS停止済み）のExecution画面を再表示してしまい、ユーザーを混乱させる |
| 戻るボタンとシステムBackを別々の遷移ロジックとして個別実装する | 挙動が食い違う（片方は安全に戻り、片方は完了済みExecutionへ戻ってしまう）事態を招くリスクがある。単一のラムダへ両方を配線し構造的に一致させた |

**影響範囲**: 変更: `features/departure/DepartureViewModel.kt`（`onManualTravelMinutesChanged`拡張・`recalculate`の判定順序是正）・`features/departure/DepartureScreen.kt`（両権限カードの表示条件是正・戻るボタン新設）・`navigation/ActionStarterNavHost.kt`（`DepartureRoute`への`navController`引き回し・`onNavigateBack`実配線・`BackHandler`新設）・`values(-ja)/strings.xml`（新規1キー`departure_back_button_label`）・`i18n/StringResourceParityTest.kt`（キー数ピン140→141）・`features/DepartureRoutingViewModelTest.kt`（T-DEPFIX-1〜5新規5件）・`features/DepartureRoutingScreenTest.kt`（T-DEPFIX-6a/6b/7新規3件・既存tPerm3_3のviewport是正）・`navigation/NavigationFlowTest.kt`（T-DEPFIX-8新規1件）・`features/AccessibilitySemanticsTest.kt`／`features/FontScaleResilienceTest.kt`（既存テストのviewport是正、アサーション内容は不変）。既存無改修: `DepartureUiState.kt`（新規フィールド不要）・`TravelTimeInput.kt`（呼び出し元の消費ロジックのみ変更）・Routes API／geocode／位置サービス各実装（スコープ外）・`isStartNavigationEnabled`関連（Phase 1未実装のまま、本修正のスコープ外として不可触を維持）。

**検証方法**: Red段階（コミット前）で`:app:testDebugUnitTest`が819 tests／failures=7（意図的Red）／errors=0／skipped=1であることを確認し、期待失敗リスト（T-DEPFIX-1/2/4/5/6b/7/8）と完全一致することを個別に照合した（born-green2件〔T-DEPFIX-3・6a〕の理由も個別に検証・報告済み）。Green実装後は`:app:testDebugUnitTest` tests=819／failures=0／errors=0／skipped=1（`app/build/test-results/**/*.xml`集計で確認）、`:app:lintDebug --rerun-tasks` error=0／warning=23（`MissingTranslation`0件を個別確認）、`:app:compileDebugAndroidTestKotlin` BUILD SUCCESSFULを確認した。Green移行時に3件（`tPerm3_3`／`tP11a12e`／`tP11f4`）が新設の戻るボタンによるレイアウト高さ増加でビューポート外に押し出され失敗したが、JUnit XMLの実際のエラーメッセージ（"is not displayed!"、ノード自体は存在）で純粋なレイアウト起因と確認し、アサーション内容を変更せず`performScrollTo()`追加のみで解消した（テストを曲げていない）。実機受け入れ（計画書§10）は本ADR起票時点で未実施のまま残る（オーケストレーターが本ADR起票後に実施）。

**再検討トリガー**: (a) 将来Developer Settings画面（仕様§36）が別目的（例: Basic/AI比較実験）で実装される場合、本フェーズが「作らない」と判断した経緯を参照する。(b) Departure routeへ新しい遷移元（Execution完了以外の経路）が追加された場合、戻り先設計（決定4、「遷移元が1経路のみ」という前提）を再評価する。(c) 上限バリデーションなしの手入力ETA（決定1・代替案却下）が実機で極端な値によるUX上の問題を引き起こした場合、バリデーション追加を再検討する。

### ADR-0067: Phase 12「Basic/AI比較実験」の結論を確定する — 端末内Qwen0.6Bは文脈適合度でBasic固定文言にnet-negative（助17%＜壊27%）、AI既定OFF維持

- 日付: 2026-08-12 ／ ステータス: 採用（`docs/plans/phase12-basic-ai-experiment.md`の3系統レビュー・ユーザー承認後、C1実装＋C2実機実験・人手評価完了をもって確定） ／ 決定者: domain-implementer（C1実装完了・C2実機実験結果をオーケストレーターが確認した時点の判断） ／ 起案agent: domain-implementer ／ 関連仕様§: §76「Basic/AI Experiment」・§36「Basic vs Local AI比較」・§55「MVP KPI」・§56「Basic vs AI KPI」

- **ADR番号の付番根拠**: 起票直前に`grep -n "^### ADR-" DECISIONS.md | tail -3`を再実行し、最新確定ADRがADR-0066（本書1737行、出発画面欠陥修正）であることを確認した。計画書`phase12-basic-ai-experiment.md`は起案時点で特定の前提ADR番号を明記していないため、上記grep結果と齟齬なくADR-0067として確定する。

**背景**: Phase 10のADR-0049決定5・ADR-0065が導入した`AnalyticsStore`／`AiResult`／`AiMetrics`基盤（Phase 9）を用い、「Basic固定文言 vs AI文脈化文言」の価値を初めて定量的に比較した。3系統レビュー（オーケストレーター＋Gemini 3.5-flash＋独立検証役）を経て、中核をandroidTestプローブ（`BasicAiComparisonProbeTest`、Room非依存で`AiResult`を直読）＋固定30イベント（事前登録、3層×10件）へ簡素化し、主指標はユーザー盲検採点（14件）・副指標は機械代理指標（30件全数）というハイブリッド構成で実験した。

**決定**:
1. **Phase 12実験結論を確定する**: A54実機・Qwen0.6B（auto選択）で固定30イベントを実行した結果、AI成功14/30（47%）・Fallback16/30（53%）。AI成功14件の盲検人手評価は良い5件（17%）・微妙1件・おかしい（有害）8件（27%）。全30件で見ると良い17%・中立57%・**有害27%**——AIが助けるより壊す方が多く、Basic固定文言に対し**net-negative**と結論する。事前登録の結論基準（計画書§10）の「0.6Bでは価値不明」（適合度がBasicと同等以下、またはエコー/reject率が高い）に該当し、実測はそれよりさらに明確な「net-negative」という結果だった。
2. **AI既定OFFを維持する**（既定ON昇格は行わない）: 有害率27%という実測結果のもとでの既定ON化は正当化できない。
3. **価値の可能性が残るのは高RAM端末のGemma4のみとし、将来課題として持ち越す**: 日常使用のA54はメモリ不足でGemma4を利用できないことがPhase 8.5§12で実測済みであるため、本フェーズでの同時検証はスコープ外とし、Gemma4／将来モデルでの再測定を将来課題とする。
4. **R1a→R1b拡張の必要性を実データで確定する**: 「おかしい」8件の大半はfew-shot seedのタイトル「歯科検診」への鸚鵡返し（例:「歯科検診を受ける場所を確認する」）だったが、この文言はseedタイトルの完全コピーではなくseedのstep本文を意味的に踏襲した言い換えであるため、R1a（`checkFewShotEcho`、タイトル集合との一致判定）をすり抜け"Applied"判定された。Phase 12計画書§3.3が事前に警告していたR1b盲点（step動作文言の逐語/意味的コピーは機械検出されない）が実データで確定した（独立検証役#4・Gemini C-2の指摘の実証）。R1b実装自体は本ADRのスコープ外（Phase 12は測定・可視化フェーズ）とし、将来のAI品質改善フェーズへの実装根拠として記録する。
5. **仕様§56の4行動ファネル指標は引き続き将来の実利用フェーズへ委譲する**: 本フェーズのプローブ環境（起点イベント自体が存在しない・編集機能が未実装・Recovery受容率は常に100%になる人工物）では測定不能という計画書§2・§13の判断を、実験完了後も維持する。

**代替案と却下理由**:

| 代替案 | 却下理由 |
|---|---|
| AI既定ONへ昇格する | 有害率27%（助ける17%を上回る）という実測結果のもとでは正当化できない。§19「AI OFF時でも動作すること」の原則にも反し、Basic版のみで価値のあるUXを壊すリスクを負う |
| F-1（few-shotカテゴリ条件選択）を本番再投入する | ADR-0064決定3でPhase 9.5の2連続A/B陰性により既に撤退済み。今回の実データでも主要故障モードは「歯科検診」への鸚鵡返し（エコー系統の欠陥）であり、カテゴリ条件選択（プレフィル短縮・エコー素材根絶が狙いだった）が今回の欠陥を直接解決する根拠はない。再投入は不支持と判断する |
| R1bを本ADRと同時に実装する | Phase 12は測定・可視化フェーズであり「AIをより賢くする」施策は計画書§3.6でスコープ外と確定済み。R1bの実装は今回確定した実データを根拠として、将来のAI品質改善フェーズで着手する |
| 「0.6Bでは価値不明」という当初の事前登録ラベルのまま報告する | 実測は「Basicと同等以下」よりも踏み込んだ「Basicを下回る（net-negative）」という結果であり、ハッジ表現より実態に即した結論を平明に報告する方が正直である |

**影響範囲**: 変更なし（Phase 12はandroidTest`@Ignore`既定のプローブとJVMのデータセット構造検証のみで構成され、本番コード・本番挙動への影響はない）。`docs/plans/phase12-basic-ai-experiment.md`§14に実験結果を記録。既存無改修: `AnalyticsStore`／`AiResult`／`AiMetrics`／`ContentSanityChecker`（Phase 9/10の既存実装をそのまま読み取りに使用、変更なし）。

**検証方法**: C1（プローブ＋データセットテスト）は`:app:testDebugUnitTest`全819件Green・lint 0を確認済み（コミット`6ebfc55`）。C2（実機実験）はA54（SCG21）で`BasicAiComparisonProbeTest`を実行し、機械副指標（appliedCount=14/30・echoCount=6・rejectReasonCounts等）をLogcat経由で収集、オーケストレーターが集計した（logcat全文は`build/agent-logs/phase12-comparison-logcat.log`、gitignore対象のため本ADR・計画書には集計数値のみを記録）。人手評価はオーケストレーターが作成した盲検シート（14件、甲/乙シャッフル）をユーザーが3軸で採点した（n=14・単一評価者、統計的有意性ではなく方向性を示す評価として扱う、計画書§3.3の事前registered方針どおり）。

**再検討トリガー**: (a) Gemma4／将来モデルでの同種実験（決定3）を実施する場合、本ADRの測定方法論（固定30イベント・ハイブリッド指標）をそのまま再利用できるかを再評価する。(b) R1b（決定4）を実装する場合、本ADRが記録した具体的な故障事例（「歯科検診」step本文への鸚鵡返し）を回帰テストの実例接地に用いる。(c) 仕様§56のファネル指標を測定できる実利用フェーズ（決定5）が到来した場合、行動ログ（`AnalyticsStore`、Phase 10）へ必要なイベント型・編集機能実装状況を再確認したうえで測定基盤を設計する。(d) 人手評価n=14・単一評価者という規模の限界により、本ADRの「net-negative」という結論が今後より大規模な評価で覆る可能性は残る——覆った場合は決定1・2を再評価する。
